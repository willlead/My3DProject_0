...
bool	ParserASE::LoadHelperObject()
{
	return true;
}
...
bool ParserASE::LoadMaterial()
{
	...
	int size = sizeof(AseMaterialTokens) / sizeof(AseMaterialTokens[0]);

	for (int index = 0; index < size; index++)
	{
		if (3 == FindWord2(AseMaterialTokens[index], index))
		{
			m_pFile = m_pFile + 1;
		}
	}
	...
	for (i = 0; i < m_ASE.MaterialList.size(); i++)
	{
		if (!FindWord(L"*MATERIAL")) 
			return false;
		if (!FindWord(L"*MATERIAL_NAME")) 
		...
		{
			if (!FindWord(L"*SUBMATERIAL")) 
				return false;
			_stscanf(m_szBuffer, _T("%s%d"), m_szWord, &iTemp);
			//if (!iTemp == iLoop) return false; // 확인용

			if (!FindWord(L"*MAP_SUBNO")) 
				return false;
...
bool ParserASE::LoadGeomObject()
{
	...
	if (!FindWord(L"*MESH_CVERTLIST")) return false;
	iMax = m_ASE.ObjectList[GeomCount].ColVertexList.size();
	for (iLoop = 0; iLoop < iMax; iLoop++)
	{
		if (!FindWord(L"*MESH_VERTCOL")) return false;
		_stscanf(m_szBuffer, _T("%s%d%f%f%f"), m_szWord, &iTemp,
			&m_ASE.ObjectList[GeomCount].ColVertexList[iLoop].x,
			&m_ASE.ObjectList[GeomCount].ColVertexList[iLoop].y,
			&m_ASE.ObjectList[GeomCount].ColVertexList[iLoop].z);
		m_ASE.ObjectList[GeomCount].ColVertexList[iLoop].w = 1.0f;  //col.a값 채우기
	...
	if (!FindWord(L"*MESH_NUMCVFACES")) return false;
	_stscanf(m_szBuffer, _T("%s%d"), m_szWord, &iTemp);
	m_ASE.ObjectList[GeomCount].ColFaceList.resize(iTemp);
	iMax = m_ASE.ObjectList[GeomCount].ColFaceList.size();
	if (!FindWord(L"*MESH_CFACELIST")) return false;
	for (iLoop = 0; iLoop < iMax; iLoop++)
	{
		if (!FindWord(L"*MESH_CFACE")) return false; //color index
		_stscanf(m_szBuffer, _T("%s%d%d%d%d"), m_szWord, &iTemp, //&v3Temp[0], &v3Temp[2], &v3Temp[1]);
			&m_ASE.ObjectList[GeomCount].ColFaceList[iLoop].i[0],
			&m_ASE.ObjectList[GeomCount].ColFaceList[iLoop].i[2],
			&m_ASE.ObjectList[GeomCount].ColFaceList[iLoop].i[1]);
	}
...
#pragma region Convert
	...
	for (iLoop = 0; iLoop < m_ASE.ObjectList[GeomCount].FaceList.size() * 3; iLoop++)
	{
		...
		m_ASE.m_VertexList[iLoop].c
			= m_ASE.ObjectList[GeomCount].ColVertexList[m_ASE.ObjectList[GeomCount].ColFaceList[iLoop / 3].i[iLoop % 3]];
		//if (m_ASE.ObjectList[GeomCount].ColFaceList[iLoop / 3].i[iLoop % 3]== 856)
		//	int i  = 0;
		//t
...
#pragma region MTLID
	...
	// submtl.size() 만큼 resize();
	m_pnctBuf.resize(5);
	int i = 0;
	for (int ipnct = 0; ipnct < m_ASE.ObjectList[0].FaceList.size(); ipnct++)
	{
		//MTLID -> PNCTBUFFER[0...]  
		//m_ASE.ObjectList[0].FaceList[0].i[3]; //1
		{
			//pnctbuf[0].PNCT.push_back( obj[0].pnct[0...]  )
			m_pnctBuf[m_ASE.ObjectList[0].FaceList[ipnct].i[3]].m_VertexList.push_back(m_ASE.m_VertexList[i++]);
			m_pnctBuf[m_ASE.ObjectList[0].FaceList[ipnct].i[3]].m_VertexList.push_back(m_ASE.m_VertexList[i++]);
			m_pnctBuf[m_ASE.ObjectList[0].FaceList[ipnct].i[3]].m_VertexList.push_back(m_ASE.m_VertexList[i++]);
		}
	}
