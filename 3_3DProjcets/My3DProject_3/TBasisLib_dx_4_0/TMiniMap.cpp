...
bool TMiniMap::BeginRender(ID3D11DeviceContext*    pContext)
{
	float ClearColor[4] = { 1.0f, 1.0f, 1.0f, 1.0f };
	if (m_pMiniMapRT->Begin(pContext, ClearColor) == false)
	{
...
{
	m_pPlaneShape->m_Object.g_pTextureSRV = m_pMiniMapRT->m_pSRV;
	pContext->RSSetViewports(1, &m_vp);
	m_pPlaneShape->SetMatrix(NULL, NULL, NULL);
	pContext->UpdateSubresource(m_pPlaneShape->m_Object.g_pConstantBuffer.Get(), 0, NULL, &m_pPlaneShape->m_cbData, 0, 0);
