...
#pragma region VB
	...
	for (int vb = 0; vb < m_ParserASE.m_pnctBuf.size(); vb++)
	{
		//iMax = m_pnctBuf[vb].m_VertexList.size();
		PNCT_VERTEX* vertices;
		vertices = (PNCT_VERTEX *)malloc(sizeof(PNCT_VERTEX) * m_ParserASE.m_pnctBuf[vb].m_VertexList.size());
		for (int i = 0; i < m_ParserASE.m_pnctBuf[vb].m_VertexList.size(); i++)
		{
			vertices[i] = m_ParserASE.m_pnctBuf[vb].m_VertexList[i];
		}

		//
		ZeroMemory(&bd, sizeof(bd));
		bd.Usage = D3D11_USAGE_DEFAULT;
		bd.ByteWidth = sizeof(PNCT_VERTEX) * m_ParserASE.m_pnctBuf[vb].m_VertexList.size(); // 개수 변경
		...
		g_pd3dDevice->CreateBuffer(&bd, &InitData, &m_ParserASE.m_pnctBuf[vb].m_pVertexBuffer);
...
#pragma region Tex
	...
	m_TexSRV.resize(5);
	//// Load the Texture  //filename으로 읽어서 값을 넣어야하는데...
	hr = D3DX11CreateShaderResourceViewFromFile(g_pd3dDevice, L"0_st02_sc00_g04.dds", NULL, NULL, &m_TexSRV[0], NULL);
	if (FAILED(hr))
		return hr;
	hr = D3DX11CreateShaderResourceViewFromFile(g_pd3dDevice, L"0_st02_sc00_g00.dds", NULL, NULL, &m_TexSRV[1], NULL);
	if (FAILED(hr))
		return hr;
	hr = D3DX11CreateShaderResourceViewFromFile(g_pd3dDevice, L"0_st02_sc00_g01.dds", NULL, NULL, &m_TexSRV[2], NULL);
	if (FAILED(hr))
		return hr;
	hr = D3DX11CreateShaderResourceViewFromFile(g_pd3dDevice, L"0_st02_sc00_g03.dds", NULL, NULL, &m_TexSRV[3], NULL);
	if (FAILED(hr))
		return hr;
	hr = D3DX11CreateShaderResourceViewFromFile(g_pd3dDevice, L"0_st02_sc00_g02.dds", NULL, NULL, &m_TexSRV[4], NULL);
	if (FAILED(hr))
		return hr;
...
#pragma region Tex ClearDepthStencilView
...
#pragma endregion 
	...
	//g_pImmediateContext->PSSetShaderResources(0, 1, &m_TexSRV[0]);
	//g_pImmediateContext->PSSetShaderResources(0, 1, &m_TexSRV[1]);
	//g_pImmediateContext->PSSetShaderResources(0, 1, &m_TexSRV[2]);
	//g_pImmediateContext->PSSetShaderResources(0, 1, &m_TexSRV[3]);
	//g_pImmediateContext->PSSetShaderResources(0, 1, &m_TexSRV[4]);
	...
	for (int i = 0; i < 5; i++)
	{
		...
		g_pImmediateContext->IASetVertexBuffers(0, 1, &m_ParserASE.m_pnctBuf[i].m_pVertexBuffer, &stride, &offset);
		g_pImmediateContext->PSSetShaderResources(0, 1, &m_TexSRV[i]);		
		g_pImmediateContext->Draw(m_ParserASE.m_pnctBuf[i].m_VertexList.size(), 0);
	}
