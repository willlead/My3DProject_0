...
public:
	...
	TMatrix ViewLookAt(	 Vector3& vPosition,
						 Vector3& vTarget,
						 Vector3& vUp )
	{
		TMatrix matrix;
	...
	TMatrix	InverseGeneral( float *out = NULL );
	TMatrix	Inverse( float *out = NULL );

	// 뷰 행렬 계산
	TMatrix ComputeViewMatrix(	Vector3& vPosition,
								Vector3& vTarget,
								Vector3& vUp)
	{
	...
	// 원근 투영 행렬 계산
	TMatrix PerspectiveLH(	float fNearPlane,
							float fFarPlane,
							float fWidth,
							float fHeight )
	{
	...
	TMatrix AxisAngle( Vector3& vAxis, float fRadian )
	{
		TMatrix matrix;
