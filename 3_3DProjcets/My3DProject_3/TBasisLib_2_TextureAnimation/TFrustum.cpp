...
void TFrustum::ExtractPlanesD3D( T_PLANE* pPlanes, const D3DXMATRIX & comboMatrix)
{
	...
	// 원점을 향하는 방향벡터( 프로스텀 내부로 향하는 벡터가 된다.)
	pPlanes[0].Normalize(); // left
	pPlanes[1].Normalize(); // rigt
	pPlanes[2].Normalize(); // top
	pPlanes[3].Normalize(); // bottom
	pPlanes[4].Normalize(); // front
	pPlanes[5].Normalize();	// back
}
bool TFrustum::CreateFrustum()
{
	...
	//ExtractPlanesD3D함수를 사용하지 않을 경우에 주석을 풀어 사용한다.
...
bool TFrustum::CreateRenderBox(ID3D11Device*  pd3dDevice, ID3D11DeviceContext* pImmediateContext)
{
	...
	if( FAILED( m_pBoxShape->Create(pd3dDevice,pImmediateContext) ) )
	{
	...
	if(m_pd3dDevice)
	{
		...
		m_pd3dDevice->CreateBlendState( &BlendState, &m_pAlphaBlend );
	}
	return true;
}
bool TFrustum::Render(ID3D11DeviceContext*    pImmediateContext)
{
    ...
    {
		// 앞면
        { m_vFrustum[1],	D3DXVECTOR4( 1.0f,0.0f, 0.0f, 0.2f ),D3DXVECTOR2( 0.0f, 0.0f)  },
        { m_vFrustum[2],  D3DXVECTOR4( 1.0f,0.0f, 0.0f, 0.2f ),D3DXVECTOR2( 1.0f, 0.0f)  },
        { m_vFrustum[3],  D3DXVECTOR4( 1.0f,0.0f, 0.0f, 0.2f ),D3DXVECTOR2( 1.0f, 1.0f)  },
        { m_vFrustum[0], D3DXVECTOR4( 1.0f,0.0f, 0.0f, 0.2f ),D3DXVECTOR2( 0.0f, 1.0f)  },

		// 뒷면
        { m_vFrustum[6], D3DXVECTOR4( 0.0f,1.0f, 0.0f, 0.2f ),D3DXVECTOR2( 0.0f, 0.0f)  },
        { m_vFrustum[5],  D3DXVECTOR4( 0.0f,1.0f, 0.0f, 0.2f ),D3DXVECTOR2( 1.0f, 0.0f)  },
        { m_vFrustum[4],  D3DXVECTOR4( 0.0f,1.0f, 0.0f, 0.2f ),D3DXVECTOR2( 1.0f, 1.0f)  },
        { m_vFrustum[7],  D3DXVECTOR4( 0.0f,1.0f, 0.0f, 0.2f ),D3DXVECTOR2( 0.0f, 1.0f)  },

		// 왼쪽
        { m_vFrustum[5], D3DXVECTOR4( 0.0f,0.0f, 1.0f, 0.2f ),D3DXVECTOR2( 0.0f, 0.0f)  },
        { m_vFrustum[1],  D3DXVECTOR4( 0.0f,0.0f, 1.0f, 0.2f ),D3DXVECTOR2( 1.0f, 0.0f)  },
        { m_vFrustum[0],  D3DXVECTOR4( 0.0f,0.0f, 1.0f, 0.2f ),D3DXVECTOR2( 1.0f, 1.0f)  },
        { m_vFrustum[4],  D3DXVECTOR4( 0.0f,0.0f, 1.0f, 0.2f ),D3DXVECTOR2( 0.0f, 1.0f)  },

		// 오늘쪽면
        { m_vFrustum[2], D3DXVECTOR4( 1.0f,1.0f, 0.0f, 0.2f ),D3DXVECTOR2( 0.0f, 0.0f)  },
        { m_vFrustum[6],  D3DXVECTOR4( 1.0f,1.0f, 0.0f, 0.2f ),D3DXVECTOR2( 1.0f, 0.0f)  },
        { m_vFrustum[7],  D3DXVECTOR4( 1.0f,1.0f, 0.0f, 0.2f ),D3DXVECTOR2( 1.0f, 1.0f)  },
        { m_vFrustum[3],  D3DXVECTOR4( 1.0f,1.0f, 0.0f, 0.2f ),D3DXVECTOR2( 0.0f, 1.0f)  },

		// 윗면
        { m_vFrustum[5], D3DXVECTOR4( 0.0f,1.0f, 1.0f, 0.2f ),D3DXVECTOR2( 0.0f, 0.0f)  },
        { m_vFrustum[6],  D3DXVECTOR4( 0.0f,1.0f, 1.0f, 0.2f ),D3DXVECTOR2( 1.0f, 0.0f)  },
        { m_vFrustum[2],  D3DXVECTOR4( 0.0f,1.0f, 1.0f, 0.2f ),D3DXVECTOR2( 1.0f, 1.0f)  },
        { m_vFrustum[1],  D3DXVECTOR4( 0.0f,1.0f, 1.0f, 0.2f ),D3DXVECTOR2( 0.0f, 1.0f)  },

		// 아랫면
        { m_vFrustum[0], D3DXVECTOR4( 0.0f,1.0f, 0.0f, 0.2f ),D3DXVECTOR2( 0.0f, 0.0f)  },
        { m_vFrustum[3],  D3DXVECTOR4( 0.0f,1.0f, 0.0f, 0.2f ),D3DXVECTOR2( 1.0f, 0.0f)  },
        { m_vFrustum[7],  D3DXVECTOR4( 0.0f,1.0f, 0.0f, 0.2f ),D3DXVECTOR2( 1.0f, 1.0f)  },
        { m_vFrustum[4],  D3DXVECTOR4( 0.0f,1.0f, 0.0f, 0.2f ),D3DXVECTOR2( 0.0f, 1.0f)  },
        
    };

	pImmediateContext->OMSetBlendState( m_pAlphaBlend, 0, 0xffffffff );	
	pImmediateContext->UpdateSubresource( m_pBoxShape->GetVB(), 0, NULL, &vertices, 0, 0); 
