...
HRESULT TTexture::Apply( ID3DX11EffectShaderResourceVariable* pTextureVariable)
{	
	return pTextureVariable->SetResource( m_pTextureRV ); 
}
...
INT TTextureMgr::Add( const TCHAR *pFileName )
{
	...
	if( FAILED( pPoint->Load( m_pd3dDevice,m_pImmediateContext, szFileName ) ))
	{
		return 0;
	}
...
INT TTextureMgr::Add( const TCHAR *pFileName, const TCHAR* szPath )
{
	TCHAR szFileName[MAX_PATH];
	TCHAR Drive[MAX_PATH];
	TCHAR Dir[MAX_PATH];
...
void TTextureMgr::SetDevice(ID3D11Device*	 pDevice,ID3D11DeviceContext*    pImmediateContext ) 
{
