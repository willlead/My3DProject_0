...
D3DXMATRIX	TCamera::SetObjectView( D3DXVECTOR3 vMax, D3DXVECTOR3 vMin )
{
	...
	D3DXVECTOR3 vPos = (m_vLookVector *  (fRadius*3) ) * -1.0f;
...
D3DXMATRIX TCamera::Update(float fElapseTime)
{
	if( ( g_InputData.bLeftClick && ( m_nRotateCameraButtonMask & MOUSE_LEFT_BUTTON) )||
		( g_InputData.bMiddleClick && ( m_nRotateCameraButtonMask & MOUSE_MIDDLE_BUTTON)) ||
		( g_InputData.bRightClick && ( m_nRotateCameraButtonMask & MOUSE_RIGHT_BUTTON)   ))
	{
		m_fCameraYawAngle	+=		DegreeToRadian( g_InputData.iMouseValue[0]* 0.1f  );
		m_fCameraPitchAngle	+=		DegreeToRadian( g_InputData.iMouseValue[1]* 0.1f  );		
	}		
	// 카메라 이동 거리 = 속도 + ( 경과시간 * 마우스 휠 변위값 )
	float fDistance =  m_fSpeed * g_InputData.iMouseValue[2]*0.1f;
	
	if( g_InputData.bSpace )	m_fSpeed += I_Timer.GetSPF() * 10.0f;
	else						m_fSpeed -= I_Timer.GetSPF() * 10.0f;
	// 최소값으로 고정
	if( m_fSpeed < 1.0f ) m_fSpeed = 1.0f;

	if( g_InputData.bWKey ) 	MoveLook(I_Timer.GetSPF() * 5.0f * m_fSpeed);
	if( g_InputData.bSKey )		MoveLook(-I_Timer.GetSPF() * 5.0f* m_fSpeed);
	if( g_InputData.bDKey )		MoveSide(I_Timer.GetSPF() * 5.0f* m_fSpeed);
	if( g_InputData.bAKey )		MoveSide(-I_Timer.GetSPF() * 5.0f* m_fSpeed);
	if( g_InputData.bQKey )		MoveUp(I_Timer.GetSPF() * 5.0f* m_fSpeed);
	if( g_InputData.bEKey )		MoveUp(-I_Timer.GetSPF() * 5.0f* m_fSpeed);
