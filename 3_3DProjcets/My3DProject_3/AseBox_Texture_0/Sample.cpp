...
#pragma endregion 
	...
	PCT_VERTEX* vertices;
	vertices = (PCT_VERTEX *)malloc(sizeof(PCT_VERTEX) * m_ParserASE.m_ASE.m_VertexList.size());
	...
	bd.ByteWidth = sizeof(PCT_VERTEX) * m_Object.ObjectList[0].FaceList.size()*3; // 개수 변경
...
#pragma endregion 
	...
	//g_View = D3DXMatrixLookAtLH(Eye, At, Up);
	D3DXMatrixLookAtLH(&g_View, &Eye, &At, &Up);
