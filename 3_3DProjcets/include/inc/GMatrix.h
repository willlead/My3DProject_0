...
public:
	...
	GMatrix operator * ( GMatrix const &Matrix )
	{
		GMatrix OutMatrix;
	...
	GMatrix ViewLookAt(	 Vector3& vPosition,
						 Vector3& vTarget,
						 Vector3& vUp )
	{
		GMatrix matrix;
	...
	GMatrix	InverseGeneral( float *out = NULL );
	GMatrix	Inverse( float *out = NULL );

	// 뷰 행렬 계산
	GMatrix ComputeViewMatrix(	Vector3& vPosition,
								Vector3& vTarget,
								Vector3& vUp)
	{
	...
	// 원근 투영 행렬 계산
	GMatrix PerspectiveLH(	float fNearPlane,
							float fFarPlane,
							float fWidth,
							float fHeight )
	{
		...
		GMatrix ret;
		ZeroMemory( this, sizeof(GMatrix));
	...
	// 원근 투영 행렬 계산
	GMatrix PerspectiveFovLH(	float fNearPlane,
								float fFarPlane,
								float fovy,
	...
	GMatrix AxisAngle( Vector3& vAxis, float fRadian )
	{
		GMatrix matrix;
