...
float GNoise::bias(float a, float b)
{
	return pow((float)a, (float)(log(b) / log(0.5)));
}
// 진폭의 변화량
float GNoise::Gain(float a, float b)
{
...
void	GNoise::Setup( float fValue, int& b0, int& b1, float& r0, float& r1 )
{
	b0		= ((int)fValue) & BM;
	b1		= (b0+1) & BM;
...
void GNoise::Normalize2(float v[2])
{
	float s;
...
void GNoise::Normalize3(float v[3])
{
	float s;
...
void GNoise::Set(int octaves,float freq,float amp,int seed)
{
	m_iOctaves = octaves;
	m_fFrequency = freq;
	m_fAmplitude = amp;
