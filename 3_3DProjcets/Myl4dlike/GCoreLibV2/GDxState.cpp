...
namespace DX
{
	...
	ID3D11RasterizerState* GDxState::g_pRSSlopeScaledDepthBias;
	ID3D11RasterizerState* GDxState::g_pRS[6] = { 0, };
	...
	ID3D11SamplerState*  GDxState::g_pSSShadowMap = NULL;
	ID3D11SamplerState*  GDxState::g_pSS[8] = { 0, };
	...
	ID3D11DepthStencilState*	GDxState::g_pDSSDepthAlways = 0;
	ID3D11DepthStencilState*	GDxState::g_pDSS[6] = { 0, };
	...
	HRESULT GDxState::SetState(ID3D11Device*	pd3dDevice)
	{
		...
		if (FAILED(hr = pd3dDevice->CreateRasterizerState(&rsDesc, &GDxState::g_pRSSlopeScaledDepthBias)))
			return hr;
