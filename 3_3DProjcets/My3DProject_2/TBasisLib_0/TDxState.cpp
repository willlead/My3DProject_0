...
namespace DX
{
	...
	ID3D11DepthStencilState*	TDxState::g_pDSS[5] = { 0, };
	...
	ID3D11BlendState*	  TDxState::g_pBSAddBlend = 0;
	ID3D11BlendState*	  TDxState::g_pBSOneSrcBlend = 0;
	...
	HRESULT TDxState::SetState(ID3D11Device*	pd3dDevice)
	{
		...
		blendDesc.RenderTarget[0].DestBlend = D3D11_BLEND_ONE;// DEST_COLOR;
		pd3dDevice->CreateBlendState(&blendDesc, &TDxState::g_pBSAddBlend);

		blendDesc.RenderTarget[0].SrcBlend = D3D11_BLEND_ONE;// DEST_COLOR;// D3D11_BLEND_DEST_COLOR;
		blendDesc.RenderTarget[0].DestBlend = D3D11_BLEND_ZERO;// DEST_COLOR;
		pd3dDevice->CreateBlendState(&blendDesc, &TDxState::g_pBSOneSrcBlend);
	...
	{
		if (g_pColorBlend)		g_pColorBlend->Release(); g_pColorBlend = NULL;
		...
		if (g_pBSAddBlend)g_pBSAddBlend->Release(); g_pBSAddBlend = NULL;
		if (g_pBSOneSrcBlend)g_pBSOneSrcBlend->Release(); g_pBSOneSrcBlend = NULL;
